...
from dagster._core.definitions.asset_check_factories.utils import DEFAULT_FRESHNESS_SEVERITY
...
from .asset_utils import get_manifest_and_translator_from_dbt_assets
...
def build_freshness_checks_from_dbt_assets(
    ...
    Examples:
        An example of configuring a non-time-partitioned dbt asset with a freshness check:
        .. code-block:: yaml
            # in schema.yml
            models:
            - name: customers
        ...
        An example of configuring a time-partitioned dbt asset with a freshness check:
        .. code-block:: yaml
            # in schema.yml
            models:
            - name: customers
    ...
    for assets_def in dbt_assets:
        ...
        asset_resource_props = [
            dbt_resource_props
            for dbt_resource_props in manifest["nodes"].values()
            if dbt_resource_props["resource_type"] in ASSET_RESOURCE_TYPES
        ]

        for dbt_resource_props in asset_resource_props:
            ...
            severity = (
                AssetCheckSeverity(severity_str) if severity_str else DEFAULT_FRESHNESS_SEVERITY
            )
            if assets_def.partitions_def and isinstance(
                ...
                try:
                    lower_bound_delta_seconds_str = freshness_check_config[
