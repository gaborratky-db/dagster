...
# Here, we know the column schema of the asset at the time we're defining it, so we can attach it to the asset decorator
@asset(
...
To attach row count metadata to an asset, you will need to attach a numerical value to the `dagster/row_count` key in the metadata parameter of the <PyObject object="MaterializeResult" /> object returned by the asset function.
